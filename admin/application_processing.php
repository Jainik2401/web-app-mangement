...
$query = "SELECT a.*, c.name AS category_name, am.company_name AS comp_name FROM applications a LEFT JOIN categories c ON(a.category_id=c.category_id) LEFT JOIN app_manager am ON(a.app_manager_id=am.app_manager_id) " ;
...
if($sql->num_rows > 0){
     ...
     while($row = $sql->fetch_assoc()){  
      ...
      $applicationsub = array("<img class='image ml-3' src='" . BASE_URL . "upload/".$row['logo']."'>",'<div style="max-width:200px; white-space:pre-wrap;">'.$row['name'].'</div>',$row['comp_name'],$row['category_name'],$status, $delete . $edit);
